...
def add_playlist():
    ...
    if form.validate_on_submit():
        ...
        try:
            ...
            flash("Playlist name already taken! Please choose another.", "danger")
...
def add_song():
    ...
    if form.validate_on_submit():
        ...
        if Song.query.filter_by(title=title, artist=artist).first():
            flash("Song title with that artist has already been added!", "danger")
...
def add_song_to_playlist(playlist_id):
    ...
    form.song.choices = [
        (row.id, row.title)
        for row in db.session.query(Song.id, Song.title)
    ...
    if form.validate_on_submit():
        if PlaylistSong.query.filter_by(
            ...
            flash("This song is already on this playlist!", "danger")
            return render_template(
                "add_song_to_playlist.html", playlist=playlist, form=form
            )
